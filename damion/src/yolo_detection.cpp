...
void boundingCb(const darknet_ros_msgs::BoundingBoxes::ConstPtr& msg){
    ...
    for(int i = 0; i < size; i++){
        ...
        if((xmax - xmin)*(ymax - ymin) < 10000){
            ...
            damion::object_info object_info;

            object_info.latitude = 0;
            object_info.longitude = 0;
            object_info.class_name = msg->bounding_boxes[i].Class;
            
            std::string file_name = "/home/kong/catkin_ws/src/damion/Image/yolo_image";
...
int main(int argc, char** argv)
{
    ...
    yolo_info = nh.advertise<damion::object_info>("/yolo_info", 1);
